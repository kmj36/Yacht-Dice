...
int print_mainmenu(void); // 메인 메뉴
int print_turn(void); // 가위바위보로 턴 정하기
void play(int, high*, low*, high*, low*); // 플레이
...
void result(int, int); // 마지막 비교 후 결과 출력
...
int main(void) // main
{
	...
	play(turn, p1_H_s, p1_L_s, p2_H_s, p2_L_s); // 플레이
...
{
	FILE* fp = fopen("lasttime.txt", "w"); // 마지막 플레이한 시간 파일 쓰기을 위해 fp 열기
...
{
	int i, sel;
	char* timpe = (char*)malloc(sizeof(char) * 50); // 시간을 받을 배열 동적할당
	FILE* fp = fopen("lasttime.txt", "r"); // 마지막으로 플레이한 시간 가져옴
